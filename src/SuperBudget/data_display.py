...
def display_df_split_by_time(df_spend: DataFrame, date_cutoff: str) -> None:
    ...
    display(df_pre_cuttoff.sort_values("total"))
...
def display_totals(
    df_spend: DataFrame, df_income: DataFrame, str_carry_over_tag: str = "CarryOver"
) -> None:
    ...
    print(
        ...
        % (df_income["amount"].sum() - df_spend["total"].sum())
    )
